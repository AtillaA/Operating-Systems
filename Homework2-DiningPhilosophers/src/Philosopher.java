...
public class Philosopher extends Thread {
	...
	public void start_walking(int i)
	{
		try {
			int walkTime = new Random().nextInt(8000);
			sleep(walkTime);
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
	...
	public void think(int i)
	{
		...
		int thinkTime = new Random().nextInt(8000);
		sleep(thinkTime);
		}
	...
	public void put_forks(int i) 
	{
		try {
			...
			test((i + N - 1) % N);	// Check if left of current philosopher can now eat.
			test((i + 1) % N);		// Check if right of current philosopher can now eat.
	...
	public void run()
	{
		...
		start_walking(id);
		System.out.println("Philosopher [" + id + "] has arrived and waiting.");
		wait_others(id);
